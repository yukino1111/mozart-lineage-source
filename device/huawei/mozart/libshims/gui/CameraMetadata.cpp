...
/*
 * Huawei's Android 6 camera HAL was compiled against the pre-Parcelable
 * CameraMetadata layout:
 *
 *     camera_metadata_t *mBuffer; // offset 0
 *     bool mLocked;               // offset 4
 *
 * Android 9 added a Parcelable vptr and moved those fields.  Calling the
 * modern methods on the five-byte legacy object both interprets the wrong
 * field as mBuffer and writes past stack instances.  Keep the old ABI local
 * to blobs that explicitly depend on this shim.
 */
...
extern "C" void camera_metadata_dtor_legacy(LegacyCameraMetadata* metadata) {
    metadata->locked = false;
    camera_metadata_clear_legacy(metadata);
}

extern "C" camera_metadata_t* camera_metadata_release_legacy(
...
extern "C" camera_metadata_t* camera_metadata_release_legacy(
    ...
    camera_metadata_t* buffer = metadata->buffer;
    metadata->buffer = nullptr;
    return buffer;
}

extern "C" void camera_metadata_acquire_legacy(
        LegacyCameraMetadata* metadata, camera_metadata_t* buffer)
...
extern "C" int32_t camera_metadata_unlock_legacy(
        LegacyCameraMetadata* metadata, const camera_metadata_t* buffer)
